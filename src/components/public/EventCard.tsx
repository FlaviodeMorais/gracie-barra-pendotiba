...
const statusConfig: Record<string, { dot: string; badge: string; label: string }> = {
  ...
  closed: {
    dot: "bg-gray-500",
    badge: "bg-gray-700/60 text-gray-300 border-gray-600/30",
    label: "Encerrado",
  },
  finished: {
    dot: "bg-gray-600",
...
export default function EventCard({
  ...
  return (
    <Link
      ...
      className={`group flex h-full flex-col overflow-hidden rounded-xl border bg-gray-900/95 transition-all duration-200 ${
        past
          ? "border-gray-800/60 opacity-60"
          : "border-gray-800 hover:border-red-500/50 hover:bg-gray-900 active:scale-[0.99]"
      }`}
    >
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        ...
        <div className="mt-auto flex items-center justify-between gap-3 border-t border-gray-800 pt-3">
          ...
          {!past && (
            <span
              className={`inline-flex min-h-10 flex-shrink-0 items-center justify-center rounded-lg px-4 text-sm font-bold transition-colors ${
                soldOut
                  ? "bg-gray-800 text-gray-500"
                  : canRegister
                  ? "bg-red-600 text-white shadow-lg shadow-red-950/35 group-hover:bg-red-500"
